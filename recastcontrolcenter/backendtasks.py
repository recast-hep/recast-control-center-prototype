...
def uploadzenodo_request(requestuuid):
  ...
  #the request should be a supplement to the theorist's paper that generated the request
  supplement_to = []
  match = re.search("([0-9]{4}.[0-9]{4})",request_info['additional-information'])
  if match:
...
def uploadzenodo_response(rootdir,requestuuid):
  ...
  if 'doi' in analysis_info:
    supplement_to += [{"relation":"isSupplementTo","identifier":analysis_info['doi'].split(':')[-1]}]

  #upload response
  depoid = createdeposition(requestuuid,
    title = "RECAST request response {}".format(requestuuid),
    description = "RECAST request response {}".format(requestuuid),
    related_identifiers = supplement_to
  )
